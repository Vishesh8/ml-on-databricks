...
# MAGIC **Prerequisite**:
# MAGIC
# MAGIC 1. As an account-level admin or metastore admin, create catalog 'dbdemos_vishesh': 
# MAGIC
# MAGIC    ```
# MAGIC    CREATE CATALOG dbdemos_vishesh;
# MAGIC    ```
# MAGIC
# MAGIC 2. Grant the `account users` group access to the catalog and schema, and the ability to create tables and views in the schema:
# MAGIC
# MAGIC    ```
# MAGIC    GRANT USE CATALOG ON CATALOG dbdemos_vishesh TO `account users`;
# MAGIC
# MAGIC    GRANT USE SCHEMA, CREATE TABLE ON SCHEMA dbdemos_vishesh.ml to `account users`;
# MAGIC    ```
# MAGIC
# MAGIC Now, any account-level user can run the commands in this notebook.

# COMMAND ----------

# DBTITLE 1,Setup: Show your access level on the dbdemos_vishesh catalog
# MAGIC %sql
# MAGIC SHOW GRANTS ON CATALOG dbdemos_vishesh;

# COMMAND ----------

# DBTITLE 1,Setup: Show your access level on the dbdemos_vishesh.ml schema
# MAGIC %sql
# MAGIC SHOW GRANTS ON SCHEMA dbdemos_vishesh.ml;

# COMMAND ----------

# DBTITLE 1,Setup: Write data to Unity Catalog
...
spark.sql("drop table if exists dbdemos_vishesh.ml.white_wine")
spark.sql("drop table if exists dbdemos_vishesh.ml.red_wine")
white_wine.write.saveAsTable("dbdemos_vishesh.ml.white_wine")
red_wine.write.saveAsTable("dbdemos_vishesh.ml.red_wine")
...
white_wine = spark.read.table("dbdemos_vishesh.ml.white_wine").toPandas()
red_wine = spark.read.table("dbdemos_vishesh.ml.red_wine").toPandas()
...
def train_model(params):
  ...
  with mlflow.start_run(nested=True):
    ...
    train_dataset = mlflow.data.load_delta(table_name="dbdemos_vishesh.ml.churn_features")
...
spark.sql("drop table if exists dbdemos_vishesh.ml.predictions")

#Write results back to Unity Catalog from Python
results.write.saveAsTable("dbdemos_vishesh.ml.predictions")
...
mlflow.register_model(model_uri, "dbdemos_vishesh.ml.wine_model")

# COMMAND ----------

# DBTITLE 1,Share table with other users
# MAGIC %sql
# MAGIC GRANT SELECT ON dbdemos_vishesh.ml.predictions TO `account users`;
